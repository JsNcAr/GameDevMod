...
(function () {
	...
	UI.showDevelopComputerMenu = function (notification, callback) {
		...
		for (var i = 0; i < computers.length; i++) {
			var item = $("<div><img src='"+ExpPack.modPath +"/source/ccimg/{0}' style='width:200px;'/><div class='sliderTabCaption rsTmb'>".format(computers[i]) + PlatformShim.toStaticHtml("Variation {0}".localize().format(i + 1)) + "</div></div>");
			slider.append(item)
		}
	...
	var updateComputerCost = function () {
		...
		costElement.empty().append($("<span>{0}</span>".format("Cost: {0}".localize().format(UI.getShortNumberString(cost)))));
		return cost
	};
